...
If iCloud/CloudKit is available and enabled for the app, collection data can be synchronised between your Apple devices using Apple’s CloudKit infrastructure and your Apple account.
...
The app only receives the photo that you actively choose through the picker. It does not require access to your entire Photos library.
...
The camera is used to scan a barcode on a physical game case so that the barcode can be used to search for matching game information.
...
They are used by the app to communicate with the corresponding service.

The app does not send these credentials to a backend server operated by me.
...
The backup is created on the user’s device and can be accessed through Apple’s Files app.

The backup is not uploaded to a server operated by me.
...
For example, a game title search may be sent to IGDB, TheGamesDB or eBay depending on the integration being used.

These requests are made directly from the user’s device to the relevant service.
...
The app also provides a Delete All Data option that allows the user to delete their collection data stored by the app on the device.
